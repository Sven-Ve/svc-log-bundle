...
final class SvcLogBundle extends AbstractBundle
{
    ...
    public function configure(DefinitionConfigurator $definition): void
    {
        $definition->rootNode()
            ...
            ->stringNode('offset_param_name')
                ->cannotBeEmpty()
                ->defaultValue('offset')
                ->info('We use offset as url parameter. If this in use, you can choose another name')
            ->end()
            ->stringNode('data_provider')
                ->defaultNull()
                ->info('Class of your one data provider to get info about sourceType and sourceID, see documentation')
            ...
            ->arrayNode('logger')->addDefaultsIfNotSet()->info('Optional configuration for default logger, see documentation')
                ->children()
                ->booleanNode('use_logger')->defaultFalse()->info('Write log entries to default logger too')->end()
            ...
            ->arrayNode('daily_summary')->addDefaultsIfNotSet()->info('Definition of the daily summary report')
                ->children()
                    ->stringNode('definition_class')
                        ->defaultNull()
                        ->info('Class of your daily summary definition')
                        ->example('App\Service\DailySummaryDefinition')
                    ->end()
                    ->stringNode('destination_email')->defaultNull()->info('Mail address for the daily summary mail')->end()
                    ->stringNode('mail_subject')->defaultValue('Daily summary')->cannotBeEmpty()->info('Subject of the daily summary mail')->end()
                ->end()
            ->end()
            ->arrayNode('kernel_exception_logger')->addDefaultsIfNotSet()->info('Configuration for the (optional) kernel exception logger')
            ->children()
                ->booleanNode('use_kernel_logger')->defaultFalse()->info('enable the kernel exception logger')->end()
                ...
                ->booleanNode('disable_404_to_logger')->defaultFalse()->info('do not send http 404 to logger (and maybe mail)')->end()
    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        $container->services()
            ->get('Svc\LogBundle\Service\BatchHelper')
            ->arg(0, $config['enable_ip_saving'])
        ;
